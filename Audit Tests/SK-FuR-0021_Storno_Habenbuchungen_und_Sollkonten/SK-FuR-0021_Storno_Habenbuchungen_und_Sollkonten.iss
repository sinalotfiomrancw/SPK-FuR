'-------------------------------------------------------------------------------------
' Title:		SK-FuR_Storno Habenbuchungen auf Sollkonten
' CIR:		SKA
' Customer:	Sparkassen
' Created by:	KKR
...
'			Creates 2 Final Result(s)
'-------------------------------------------------------------------------------------
' Change History
'-------------------------------------------------------------------------------------
' Changed by:		MD
' Changed on:	 	07.7.2019
' Requested by: 	MG
' Comment: 		PS 10 und 16 werden zusammen zu einem PS SK-FuR 0021	
' ToDO:		Anmeldung über "AssociateSecondary" in SKA_FuR-ImportRoutine.iss 
'------------------
' Changed by:	AS
...
' tables
Dim dbStornoHabenAufSoll As String
...
'	SmartContext.Log.LogMessage "Runs on server: {0}", SmartContext.IsServerTask
	...
	'dbNameSourceAdditional = SmartContext.InputFileByAlias("SKAStornoHabenAufSollNichtEUR")
...
Dim dbStornoHabenAufSollSummiert As String
...
Dim rec As Object
	...
	'dbBuchungenJeKtoRahmen = oSC.UniqueFileName("-SKA0021_Anzahl_Buchungen_je_bebuchten_KtoRahmen_mit_SHK.IMD")
	...
	dbBuchungenJeKtoRahmen = oSC.UniqueFileName(sAuditFolder & "-SKA0021_Anzahl_Buchungen_je_KtoRahmen.IMD")
	task.OutputDBName = dbBuchungenJeKtoRahmen
	task.CreatePercentField = False
	task.StatisticsToInclude = SM_SUM
	task.DisableProgressNotification = True
	...
	'dbName = oSC.UniqueFileName(sAuditFolder & "SK-FuR-0021_Storno_HabenBuchungen_auf_SollKonten.IMD")
	dbStornoHabenAufSoll = oSC.UniqueFileName(sAuditFolder & "SK-FuR-0021_Storno_HabenBuchungen_auf_SollKonten.IMD")
	...
	task.AddExtraction dbStornoHabenAufSoll, "","AZ9_SALDO <= 0,00 .AND. BETRAG > 0,00 .AND. ( TEXTSCHLÜSSEL == ""25"" .OR. TEXTSCHLÜSSEL == ""68"" .OR.  @isini(""Storno"";VERWENDUNGSZWECK)  .OR. @isini(""Korrektur"";VERWENDUNGSZWECK)  .OR.  @isini(""Berichtigung"";VERWENDUNGSZWECK) )" & sPositionEqn ' 25.07.2022
...
'	Set oResultFile = oSC.CreateResultObject(dbName, FINAL_RESULT, True, 1)
	...
	Set oResultFile = oSC.CreateResultObject(dbStornoHabenAufSoll, INTERMEDIATE_RESULT, True, 2)
...
' Amount of rows which should be shown in the report
	...
	Call SetFlagForTable(dbStornoHabenAufSoll, TRUE)
	
	'Set db = Client.OpenDatabase(dbName)
	Set db = Client.OpenDatabase(dbStornoHabenAufSoll)
	Set task = db.Summarization
	task.AddFieldToSummarize "RAHMENNR_2STELLIG"
	task.AddFieldToTotal "BETRAG"
	task.AddFieldToInc "WKZ"
	task.Criteria = "WKZ==""EUR"""
	dbStornoHabenAufSollSummiert = oSC.UniqueFileName(sAuditFolder & "SK-FuR-0021_Storno_HabenBuchungen_auf_SollKonten_in_Euro.IMD")
	task.OutputDBName = dbStornoHabenAufSollSummiert
	task.CreatePercentField = FALSE
	...
	Set db = Client.OpenDatabase(dbStornoHabenAufSollSummiert)
	If db.Count > 0 Then
		' 14.09.2022
		...
		task.CreateVirtualDatabase = False
		task.DisableProgressNotification = True
		dbStornoHabenAufSollSummiert = oSC.UniqueFileName(sAuditFolder & "@SK-FuR-0021_Storno_HabenBuchungen_auf_SollKonten_summiert_in_EUR.IMD")
		task.PerformTask dbStornoHabenAufSollSummiert, "", WI_JOIN_ALL_IN_PRIM
		db.Close
		Set task = Nothing
		Set db = Nothing
		...
		Call removeActionField(dbStornoHabenAufSollSummiert,"ANZ_SAETZE_GESAMT")
	...
	'Final Result:
	'Set oResultFile = oSC.CreateResultObject(dbName, FINAL_RESULT, True, 1)
	Set oResultFile = oSC.CreateResultObject(dbStornoHabenAufSollSummiert, FINAL_RESULT, True, 2)
	...
	Call SetFlagForTable(dbStornoHabenAufSollSummiert, TRUE)
	
	If bPrivateMode = TRUE Then
		' Remove History in files created by Analysis
		If oSC.FileExists(dbStornoHabenAufSollSummiert, "") Then oProtectIP.RemoveHistory dbStornoHabenAufSollSummiert
	End If
...
' Frühender PS 10. Zusammenführung v1.4 
...
'	Set db = Client.OpenDatabase("dbUmsaetzeOBRManuell")
	...
	'dbName = sAuditFolder & "SK-FuR-0021_Storno_HabenBuchungen_auf_SollKonten.IMD"
	'Set db = Client.OpenDatabase(dbName)
	Set db = Client.OpenDatabase(dbStornoHabenAufSoll)
	Set task = db.Summarization
	...
	dbStornoHabenAufSollSummiert = oSC.UniqueFileName(sAuditFolder & "@SK-FuR-0021_Storno_HabenBuchungen_auf_SollKonten_nicht_in_EUR.IMD")
	task.OutputDBName = dbStornoHabenAufSollSummiert
	task.CreatePercentField = FALSE
	task.StatisticsToInclude = SM_SUM + SM_MAX + SM_MIN + SM_AVERAGE
	task.DisableProgressNotification = True
