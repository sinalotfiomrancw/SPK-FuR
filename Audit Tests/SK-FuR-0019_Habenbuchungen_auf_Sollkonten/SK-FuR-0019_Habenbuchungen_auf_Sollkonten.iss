'-------------------------------------------------------------------------------------
' Title:		SKA_019_Habenbuch_auf_Sollkto_nicht_EUR
' CIR:		SKA
' Customer:	Sparkassen
' Created by:	KKR
...
' ##############################################################################
		...
		Call Analysis() 'aus PS 07
		Call Analysis2() 'aus PS 014 
...
Dim rec As Object
	...
	'dbBuchungenJeKtoRahmen = oSC.UniqueFileName("-SKA0019_Anzahl_Buchungen_je_bebuchten_KtoRahmen_mit_SHK.IMD")
	...
	dbBuchungenJeKtoRahmen = oSC.UniqueFileName(sAuditFolder & "-SKA0019_Anzahl_Buchungen_je_KtoRahmen.IMD")
	task.OutputDBName = dbBuchungenJeKtoRahmen
	task.CreatePercentField = False
	task.StatisticsToInclude = SM_SUM
	task.DisableProgressNotification = True
	...
	'Durchzusammenführ der PS 7/14 wird die Logi aus Import-DataPreparationen übernommen
	'Aktionen basieren direkt auf "SKA00_Umsätze_zu_OBR_manuell.IMD" 
	...
	'Set db = Client.OpenDatabase("-SKA00_Umsätze_zu_OBR_manuell.IMD")
	Set db = Client.OpenDatabase(dbNameSource)
	Set task = db.Extraction
	task.IncludeAllFields
	'dbName = oSC.UniqueFileName(sAuditFolder & "SK-FuR-0019_HabenBuchungen_auf_SollKonten.IMD")
	dbHabenAufSoll = oSC.UniqueFileName(sAuditFolder & "SK-FuR-0019_HabenBuchungen_auf_SollKonten.IMD")

	'task.AddExtraction dbName, ""," AZ9_SALDO <= 0,00 .AND. BETRAG > 0,00 .AND. TEXTSCHLÜSSEL <> ""25"" .AND. TEXTSCHLÜSSEL <> ""68"" " ' AS 06.10.2020
	'task.AddExtraction dbName, ""," AZ9_SALDO < 0,00 .AND. BETRAG >= 0,00 .AND. TEXTSCHLÜSSEL <> ""25"" .AND. TEXTSCHLÜSSEL <> ""68"" " ' AS 06.10.2020: Sollkonten ohne Saldo 0,00
	...
	task.AddExtraction dbHabenAufSoll, ""," AZ9_SALDO < 0,00 .AND. BETRAG >= 0,00 .AND. TEXTSCHLÜSSEL <> ""25"" .AND. TEXTSCHLÜSSEL <> ""68"" " & sPositionEqn ' 25.07.2022
...
'	Set oResultFile = oSC.CreateResultObject(dbName, FINAL_RESULT, True, 1)
	...
	Set oResultFile = oSC.CreateResultObject(dbHabenAufSoll, INTERMEDIATE_RESULT, True, 2)
...
' Amount of rows which should be shown in the report
	...
	Call SetFlagForTable(dbHabenAufSoll, TRUE)
	
	'Set db = Client.OpenDatabase(dbNameSource)
	'Set db = Client.OpenDatabase(dbName)
	Set db = Client.OpenDatabase(dbHabenAufSoll)
	Set task = db.Summarization
	task.AddFieldToSummarize "RAHMENNR_2STELLIG"
	...
	dbHabenAufSollSummiert = oSC.UniqueFileName(sAuditFolder & "SK-FuR-0019_HabenBuchungen_auf_SollKonten_summiert_in_EUR.IMD")
	...
	If db.Count > 0 Then
		...
		task.CreateVirtualDatabase = False
		task.DisableProgressNotification = True
		dbHabenAufSollSummiert = oSC.UniqueFileName(sAuditFolder & "@SK-FuR-0019_HabenBuchungen_auf_SollKonten_in_EUR_je_KtoRahmen.IMD")
		...
		Call removeActionField(dbHabenAufSollSummiert, "ANZ_SAETZE_GESAMT")
	
		'lösche Zwischenergebniss Tabele
	Else
		db.Close
		Set db = Nothing
	End If
...
' Amount of rows which should be shown in the report
	...
	Call SetFlagForTable(dbHabenAufSollSummiert, TRUE)
...
'Zusmmenführung d PS 07 und 014 in V 1.04
Function Analysis2
' Local Variables
Dim dbName As String
Dim sFieldNameOverallCreditLimit As String
...
'	Set db = Client.OpenDatabase("dbUmsaetzeOBRManuell")
	...
	'dbName = sAuditFolder & "SK-FuR-0019_HabenBuchungen_auf_SollKonten.IMD"
	...
	dbHabenAufSollSummiert = oSC.UniqueFileName(sAuditFolder & "@SK-FuR-0019_HabenBuchungen_auf_SollKonten_nicht_in_EUR_je_KtoRahmen.IMD")
	task.OutputDBName = dbHabenAufSollSummiert
	task.CreatePercentField = FALSE
	task.StatisticsToInclude = SM_SUM + SM_MAX + SM_MIN + SM_AVERAGE
	task.DisableProgressNotification = True
