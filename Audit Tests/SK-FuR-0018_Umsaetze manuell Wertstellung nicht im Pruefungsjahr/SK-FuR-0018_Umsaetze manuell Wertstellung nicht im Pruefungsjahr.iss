'-------------------------------------------------------------------------------------
' Title:		SKA_002_Umsaetze_manuell_Wertstellung_nicht_im_Prüfjahr
' CIR:		SKA
' Customer:	Sparkassen
' Created by:	KKR
...
Dim dbUmsaetzeNichtPJahr As String
Dim dbRelUmsaetzeNichtPJahr As String 
...
Dim sUmsaetze As String ' 25.07.2022
	...
	dbVariables = sAuditFolder & "{Variables}.IMD"
	...
	dbBuchungenJeKtoRahmenGes = oSC.UniqueFileName(sAuditFolder & "-SKA018_Anzahl_Buchungen_je_KtoRahmen.IMD")
...
'	Set db = Client.OpenDatabase("dbUmsaetzeOBRManuell")
	...
	dbUmsaetzeNichtPJahr = oSC.UniqueFileName(sAuditFolder & "-SK-FuR-0018_Umsätze_manuell_Wertstellung_nicht_im_Prüfjahr.IMD")
	eqn = "WERTSTELLUNG_JAHR <>" & """" & sGJahr & """" & sPositionEqn
	task.AddExtraction dbUmsaetzeNichtPJahr , "", eqn
	...
	Set db = Client.OpenDatabase(dbUmsaetzeNichtPJahr)
	Set task = db.Summarization
	task.AddFieldToSummarize "RAHMENNR_2STELLIG"
	task.AddFieldToInc "POSITION_SHORT"
	task.AddFieldToTotal "BETRAG"
	dbUmsaetzeNichtPJahr = oSC.UniqueFileName(sAuditFolder & "-SK-FuR-0018_Umsätze_manuell_Wertstellung_nicht_im_Prüfjahr_summiert.IMD")
	task.OutputDBName = dbUmsaetzeNichtPJahr
	task.CreatePercentField = False
	...
	If db.Count > 0 Then
		...
		task.CreateVirtualDatabase = False
		task.DisableProgressNotification = True
		'dbUmsaetzeNichtPJahr = "@SKA02_Umsätze_manuell_glatte_1000er_summiert.IMD"
		dbUmsaetzeNichtPJahr = oSC.UniqueFileName(sAuditFolder & "@SK-FuR-0018_Umsätze_manuell_Wertstellung_nicht_im_Prüfjahr_summiert.IMD")
		
		task.PerformTask dbUmsaetzeNichtPJahr, "", WI_JOIN_ALL_IN_PRIM
		db.Close
		Set task = Nothing
		Set db = Nothing
...
'	Set oResultFile = oSC.CreateResultObject(dbName, FINAL_RESULT, True, 1)
	Set oResultFile = oSC.CreateResultObject(dbUmsaetzeNichtPJahr, FINAL_RESULT, True, 2)
...
' Amount of rows which should be shown in the report
	...
	Call SetFlagForTable(dbUmsaetzeNichtPJahr, TRUE)
	
	If bPrivateMode = TRUE Then
		' Remove History in files created by Analysis
		If oSC.FileExists(dbUmsaetzeNichtPJahr, "") Then oProtectIP.RemoveHistory dbUmsaetzeNichtPJahr
	End If
...
' Zusammenführung der PS 02 Und 05 ab v 1.4
' PS 05 
...
Dim sFieldNameOverallCreditLimit As String
Dim lNumberOfHistoryCountsSourceFileStart As Long
'Dim dbBuchungenJeKtoRahmenGes As String ' 25.07.2022
Dim dbRelUmsaetzeNichtPJahr  As String
Dim dbVariables As String
Dim sGJahr As String
Dim iStichprobe As Integer
...
'	Set db = Client.OpenDatabase("dbUmsaetzeOBRManuell")
	...
	Set task = db.Extraction
	task.IncludeAllFields
	dbRelUmsaetzeNichtPJahr = oSC.UniqueFileName(sAuditFolder & "@SK-FuR-0018_Umsätze_manuell_Wertstellung_nicht_im_Prüfjahr_und_größer_Nichtaufgriffsgrenze.IMD")
	eqn = "RELEVANT <> """" .AND. WERTSTELLUNG_JAHR <>" & """" & sGJahr & """" & sPositionEqn
	task.AddExtraction dbRelUmsaetzeNichtPJahr, "", eqn
	task.CreateVirtualDatabase = False
	task.DisableProgressNotification = True
	task.PerformTask 1, db.Count
	db.Close
	...
	'Final Result:
'	Set oResultFile = oSC.CreateResultObject(dbName, FINAL_RESULT, True, 1)
	Set oResultFile = oSC.CreateResultObject(dbRelUmsaetzeNichtPJahr, FINAL_RESULT, True, 2)
' Short Description (optional)dbRelUmsaetzeNichtPJahr
'	oResultFile.ExtraValues.Add "ShortName", oGetResourceString("ResultFileSKA001ShortName")
' Description
'	oResultFile.ExtraValues.Add "Description", oGetResourceString("ResultFileSKA001Description")
' Columns and sequence (optional)
...
' Amount of rows which should be shown in the report
	...
	Call SetFlagForTable(dbRelUmsaetzeNichtPJahr, TRUE)
	
	If bPrivateMode = TRUE Then
		' Remove History in files created by Analysis
		If oSC.FileExists(dbRelUmsaetzeNichtPJahr, "") Then oProtectIP.RemoveHistory dbRelUmsaetzeNichtPJahr
	End If
