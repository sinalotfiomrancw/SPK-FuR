...
ErrHandler_CheckPointID = "0.0"
   ...
   Call Scrambling("SK-FuR-0033; sWorkDir on Client: " & sWorkDir,true,true)
   ...
   SmartContext.Log.LogMessage s
   Call Scrambling("START Date And Time: " & Now() ,true,true)
...
ErrHandler_CheckPointID = "Equation for Extraction Temp0"
	...
	If Len(sAccounts) > 0 Then sEqn_0 = sEqnReserved & " .AND. (" & sAccounts & ")"	
	...
	If Len(sRahmennr) > 0 Then sEqn_0 = sEqnReserved & " .AND. (" & sRahmennr & ")"
	
	s = ErrHandler_CheckPointID & ": " 
	Call Scrambling(s,true,true)		
	s = "Reserved equation: " & sEqnReserved & ", reservedlength: " & lngReservedLength
	Call Scrambling(s,true,true)	
	s = "Built equation(sAccounts) : " & sRahmennr	
	Call Scrambling(s,true,true)
	s = "Final equation: " & sEqn_0
	Call Scrambling(s,true,true)
	...
	Set oParameters = SmartContext.Parameters
	If oParameters.Contains("SmartSingleList2") Then
		bIsSourceTable = True
		sPosition = BuildEquation_SingleList3(sSourceName, bIsSourceTable, lngReservedLength)
	End If
	Set oParameters = Nothing		
	
	If Len(sPosition) > 0 Then sEqn_0 = sEqnReserved & " .AND. (" & sPosition & ")"
	
	s = ErrHandler_CheckPointID & ": " 
	Call Scrambling(s,true,true)		
	s = "Reserved equation: " & sEqnReserved & ", reservedlength: " & lngReservedLength
	Call Scrambling(s,true,true)	
	s = "Built equation(sAccounts) : " & sPosition	
	Call Scrambling(s,true,true)
	s = "Final equation: " & sEqn_0
	Call Scrambling(s,true,true)
    
    sEqn_0 = oSC.Replace(sEqn_0,"YYYYMMDD",sDateMask)
    Call Scrambling(ErrHandler_CheckPointID & ", eqn: " & sEqn_0,true,true)        
...
ErrHandler_CheckPointID = "11.4.5 Extraction ID_Result_1"
	...
	'sdbname = s_Temp0
	sID_Result_1 = oSC.UniqueFileName(sID_Result_1, FINAL_RESULT)
	'Set oDB = Client.OpenDatabase(s_Temp0)	-> already open at line 766	
	Set oTask = oDB.Extraction
	...
	For i=1 To oTable.Count 
		...
		If Not (oField.Name =  sID_Fieldname_CriticalText Or oField.Name =  sID_Fieldname_ShortText Or oField.Name = sID_Fieldname_Existing_CriticalText) Then
			oTask.AddFieldToInc oField.Name
		End If
...
ErrHandler_CheckPointID = "MS_2a"    
    ...
    Call Scrambling(" === sDateMask: " & sDateMask, true, true)   

ErrHandler_CheckPointID = "MS_3"
   'get ListSeparator
   sLS = oSC.ListSeparator
   SmartContext.Log.LogMessage oStrings("ListSeparator") & sLS
   Call Scrambling("sDBExtension: " & sDBExtension & "; ListSeparator: " & sLS, true, true)
   
   SmartContext.Log.LogMessage oStrings("MAX_EQUATION_LENGTH"), MAX_EQUATION_LENGTH
   Call Scrambling("MAX_EQUATION_LENGTH: " & MAX_EQUATION_LENGTH, true, true)
...
ErrHandler_CheckPointID = "2.2.1"	
	...
	Call Scrambling("number of entries in Primarys history when macro starts: " & nHistoryCount,true,true)
...
ErrHandler_CheckPointID = "MV_4"
   ...
   Call Scrambling("MacroValidation; Initial number of indices on source table: " & iNrOfIndeces, true, true)
...
ErrHandler_CheckPointID = "AFN_0"
   ...
   Call Scrambling("aFiles(" & iNumberOfFiles & "): " & FileName,true,true)
...
ErrHandler_CheckPointID = "ATFN_0"
   ...
   Call Scrambling("aTempFiles(" & iNumberOfTempFiles & "): " & FileName,true,true)
...
ErrHandler_CheckPointID = "ARs_1"
	If bID_Result_1 Then
		...
		If bExistsResult Then Call Scrambling("added sResult_1", true, true)'error result handler
	End If		
...
ErrHandler_CheckPointID = "DTF_" & CStr(i)
		 sName = aTempFiles(i)
         oSC.GetRidOf(sName)
         Call Scrambling(ErrHandler_FunctionName & "temp file deleted(" & CStr(i) & "): " & sName,true,true)
   Next
   Exit Function
...
subCheckPointID = "MDH_" & CStr(i)
			...
			End If
         Call Scrambling("MacroDeleteHistory; " & "history deleted(" & CStr(i) & "): " & aFiles(i), true, true)
      Next
   ...
   Call Scrambling("MacroDeleteHistory; " & "base Tables history deleted: " & sSourceName,true,true)
...
ErrHandler_CheckPointID = "MC_1"
   ...
   Call Scrambling("Indices to be deleted on sSourceName(" & sSourceName & ") : " & sIndexes ,true,true)
   sdbname = sSourceName
   'oIndexCleaner.DeleteIndex sSourceName , sIndexes
   Call MacroDeleteIndexes(sSourceName,sIndexes)
   Call Scrambling("Indices deleted!", true,true)
   'delete tables history
   Call MacroDeleteHistory(iNumberOfFiles)

   If SmartContext.ExecutionStatus = EXECUTION_SUCCESS Then
	  Call Scrambling("Number Of Temp Files: " & iNumberOfTempFiles+1,true,true)	
      Call DeleteTempFiles(iNumberOfTempFiles)
		 
      Call Scrambling("Execution status: " & SmartContext.ExecutionStatus & "(SUCCESS)",true,true)
   ...
   Else
      ...
      Call Scrambling(FunctionName & " Clear results executed",true,true)
      Call Scrambling(FunctionName & " iNumberOfFiles:" & iNumberOfFiles+1,true,true)
      ...
      If iNumberOfFiles > 0 Then
         ...
         For j = 1 To iNumberOfFiles
            sName = aFiles(i)
            oSC.GetRidOf(sName)
            Call Scrambling(FunctionName & " file deleted(" & CStr(i) & "): " & sName,true,true)
            i = i-1
         Next
      End If   ' for "If iNumberOfFiles > 0 "

      Call Scrambling(FunctionName & " END of ROLLBACK",true,true)

   End If   ' for "if SmartContext.ExecutionStatus = EXECUTION_SUCCESS"

   If SmartContext.ExecutionStatus = EXECUTION_FAIL Then
      Call Scrambling(FunctionName & "Execution status: " & SmartContext.ExecutionStatus & "(FAIL)",true,true)
      Call Scrambling(LOG_HEADER & "; END Date And Time: " & Now(),true, true)
   ...
   If SmartContext.ExecutionStatus = EXECUTION_CANCELED Then
      Call Scrambling(FunctionName & "Audit Test Canceled By User, Execution status: " & SmartContext.ExecutionStatus & "(CANCEL)",true,true)
      Call Scrambling(LOG_HEADER & "; END Date And Time: " & Now(),true, true)
...
ErrHandler_CheckPointID = "NI_0"
   ...
   Call Scrambling(ErrHandler_CheckPointID & "; FileName: " & FileName, true, true)
    ...
    For p = 1 To Len(sNumberOfIndeces)
       ...
       End If
    Next
    Call Scrambling("NumberOfIndices: " & NumberOfIndeces, true, true)
    ErrHandler_CheckPointID = "NI_2"
    oImd.CloseIMD
    Set oImd = Nothing
    Call Scrambling("Number of indices on table: " & FileName & "; NumberOfIndeces: " & NumberOfIndeces, true, true)
...
ErrHandler_CheckPointID = "MDI_1"
      ...
      Call Scrambling("index to delete (Key1, Key2): " & Key1 & ", " & Key2, true, true)
...
ErrHandler_CheckPointID = "MDI_4"
   ...
   iCrtNrOfIndeces =  NumberOfIndeces(dbName)		'sSourceName
   Call Scrambling("Number of indices on source table after index delete: " & iCrtNrOfIndeces, true, true)
...
'------------------------------------------------------------------------------------------------------
Function BuildEquation_SingleList3(ByVal dbName As String, ByVal IsSourceTable As Boolean, ByVal ReservedLength As Long) As String
ErrHandler_CheckPointID = "BuildEquation_SingleList3_0"

	BuildEquation_SingleList3 = ""
	bToDeleteTempColumns = False		' will be true if result = 0,len(dbname)>0 and IsSourceTable=true
			
	Dim accountNumberName As String
	accountNumberName = snPositionShort
	
	Dim accountNumberRanges As Object
	Set accountNumberRanges = oParameters.Item("SmartSingleList2")
	...
	filterExpression = ""
	If result = 1 Then
ErrHandler_CheckPointID = "BuildEquation_SingleList3_1"	
		' A filter expression is available in Equation
		filterExpression = filterBuilder.Equation
		SmartContext.Log.LogMessage "Filter = {0}", filterExpression
	
		BuildEquation_SingleList3 = filterExpression
				
	ElseIf result > 0 Then
ErrHandler_CheckPointID = "BuildEquation_SingleList3_2"	
		' The filter expression exceeded the max. allowed length -> result contains the length of the expression
		' Note: In this case the Equation property of the builder is empty!
		Dim ErrorMessageFilterLength As String
		ErrorMessageFilterLength = oStrings("ErrorMessFilterExpressionTooLong")
	...
	ElseIf result = 0 Then
ErrHandler_CheckPointID = "BuildEquation_SingleList3_3"	
		' PerformTask returns 0 in case the 'regular' expression exceeded IDEA's max. equation length.
		' A filter expression was constructed, however helper columns were added to the source database.
		' The names of these columns are available in the builder's array TempColumnNames.
		' Note: To enable this behavior, the builder must be initialzed with the source database (path or database object)
