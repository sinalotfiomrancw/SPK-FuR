'-------------------------------------------------------------------------------------
' Title:		SKA_006_Habenbuchungen_auf_Sollkonten_im_KtoRahmen_92_bis_94_oberhalb_NAGrenze
' CIR:		SKA
' Customer:	Sparkassen
' Created by:	KKR
...
Dim dbHabenAufSollKto92To94 As String
...
'	Set db = Client.OpenDatabase("dbUmsaetzeOBRManuell")
	...
	dbHabenAufSollKto92To94 = oSC.UniqueFileName(sAuditFolder & "@SK-FuR-0006_HabenBuchungen_im_KtoRahmen_92_bis_94_Nichtaufgriffsgrenze.IMD")
	task.AddExtraction dbHabenAufSollKto92To94, "", "(RAHMENNR_2STELLIG == ""92"" .OR. RAHMENNR_2STELLIG == ""93"" .OR. RAHMENNR_2STELLIG == ""94"") .AND. RELEVANT == ""X""" & sPositionEqn ' 25.07.2022
...
'	Set oResultFile = oSC.CreateResultObject(dbName, FINAL_RESULT, True, 1)
	Set oResultFile = oSC.CreateResultObject(dbHabenAufSollKto92To94, FINAL_RESULT, True, 2)
...
' Amount of rows which should be shown in the report
	...
	Call SetFlagForTable(dbHabenAufSollKto92To94, TRUE)
	
	If bPrivateMode = TRUE Then
		' Remove History in files created by Analysis
		If oSC.FileExists(dbHabenAufSollKto92To94, "") Then oProtectIP.RemoveHistory dbHabenAufSollKto92To94
	End If
