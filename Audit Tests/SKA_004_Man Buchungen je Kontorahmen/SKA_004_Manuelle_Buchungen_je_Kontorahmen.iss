'-------------------------------------------------------------------------------------
' Title:		SKA_004_Manuelle_Buchungen_je_Kontorahmen
' CIR:		SKA
' Customer:	Sparkassen
' Created by:	KKR
...
Dim dbUmsaetzeManuellJeKtoQuote As String

	'dbBuchungenJeKtoRahmenGes = sAuditFolder & "-SKA00_Anzahl_Buchungen_je_KtoRahmen.IMD" ' 25.07.2022
	sUmsaetze = sAuditFolder & "-SKA00_Umsätze_zu_OBR_in EUR.IMD" ' 25.07.2022
...
'	Set db = Client.OpenDatabase("dbUmsaetzeOBRManuell")
	...
	' Create "-SKA004_Manuelle_Buchungen_je_KtoRahmen.IMD"
	...
	If db.Count > 0 Then
		...
		dbBuchungenJeKtoRahmenGes = oSC.UniqueFileName(sAuditFolder & "-SKA004_Anzahl_Buchungen_in_EUR_je_KtoRahmen.IMD")
		...
		task.AddFieldToTotal "SOLL_BETRAG"
		task.Criteria = sPositionEqn
		dbUmsaetzeManuellJeKto = oSC.UniqueFileName(sAuditFolder & "SK-FuR-0004-Manuelle Buchungen in EUR je KtoRahmen.IMD")
		...
		'Set db = Client.OpenDatabase(dbUmsaetzeManuellJeKto)
		'If db.Count > 0 Then
		'	oSC.CreateActionField db, "ANZ_SAETZE", dbNameSource, "RAHMENNR_2STELLIG"
		'End If
		'db.Close
		'Set db = Nothing
		...
		Set oResultFile = oSC.CreateResultObject(dbUmsaetzeManuellJeKto, INPUT_DATABASE, True, 1)
		SmartContext.TestResultFiles.Add oResultFile
		Set oResultFile = Nothing
		
		Call SetFlagForTable(dbUmsaetzeManuellJeKto, TRUE)
		...
		If bPrivateMode = TRUE Then
			...
			If oSC.FileExists(dbUmsaetzeManuellJeKto, "") Then oProtectIP.RemoveHistory dbUmsaetzeManuellJeKto
		End If
	...
	If db.Count > 0 Then
		...
		task.DisableProgressNotification = True
		dbUmsaetzeManuellJeKtoQuote = oSC.UniqueFileName(sAuditFolder & "@SK-FuR-0004_Manuelle_Buchungen_in_EUR_je_KtoRahmen.IMD")
		task.PerformTask dbUmsaetzeManuellJeKtoQuote, "", WI_JOIN_ALL_IN_PRIM
		db.Close
		Set task = Nothing
		Set db = Nothing
		SmartContext.ScriptProgress 26, 45
		Set db = Client.OpenDatabase(dbUmsaetzeManuellJeKtoQuote)
	...
	'Set db = Client.OpenDatabase(dbUmsaetzeManuellJeKtoQuote)
	'If db.Count > 0 Then
	'	oSC.CreateActionField db, "ANZ_SAETZE_MANUELL", dbNameSource, "RAHMENNR_2STELLIG"
	'	oSC.CreateActionField db, "ANZ_SAETZE", sUmsaetze, "RAHMENNR_2STELLIG"
	'End If
	'db.Close
	'Set db = Nothing
...
'	Set oResultFile = oSC.CreateResultObject(dbName, FINAL_RESULT, True, 1)
	Set oResultFile = oSC.CreateResultObject(dbUmsaetzeManuellJeKtoQuote, FINAL_RESULT, True, 2)
...
' Amount of rows which should be shown in the report
	oResultFile.ExtraValues.Add "RowCount", 24
	SmartContext.TestResultFiles.Add oResultFile
	Set oResultFile = Nothing
	
	Call SetFlagForTable(dbUmsaetzeManuellJeKtoQuote, TRUE)
	
	If bPrivateMode = TRUE Then
		' Remove History in files created by Analysis
		If oSC.FileExists(dbUmsaetzeManuellJeKtoQuote, "") Then oProtectIP.RemoveHistory dbUmsaetzeManuellJeKtoQuote
	End If
	
	'If oSC.FileExists(dbTempBuchungenInEURGes, "") Then Kill dbTempBuchungenInEURGes
	'If oSC.FileExists(dbBuchungenJeKtoRahmenGes, "") Then Kill dbBuchungenJeKtoRahmenGes
