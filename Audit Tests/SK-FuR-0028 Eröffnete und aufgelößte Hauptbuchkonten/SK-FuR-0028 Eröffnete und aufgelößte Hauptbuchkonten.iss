'-------------------------------------------------------------------------------------
' Title:		SKA-FuR-0028 Eröffnete und aufgelöste Hauptbuchkonten
' CIR:		SKA_FuR
' Customer:	Sparkassen
' Created by:	AS
...
' Results
dim sEröffnungen as string
Dim sAuflösungen As String
Dim sEröffnungenFolgeJahr As String
Dim sAuflösungenFolgeJahr As String
...
SetCheckpoint "Begin of Functions"
	...
	Call registerResult(sEröffnungen, FINAL_RESULT)
	call registerResult(sAuflösungen, FINAL_RESULT)
	'03.08.2022
	Call registerResult(sEröffnungenFolgeJahr, FINAL_RESULT)
	Call registerResult(sAuflösungenFolgeJahr, FINAL_RESULT)
...
SetCheckpoint "analysis, Checkpoint 1.0"
	...
	sEröffnungen = oSC.UniqueFileName("Eröffnungen im aktuellen Bilanzjahr.IMD", FINAL_RESULT)
	task.AddExtraction sEröffnungen, "", "ERÖFFNUNG >= """ & sAktuelleGJAHR & "0101""  .AND. ERÖFFNUNG < """ & sAktuelleGJAHR & "1231""" 'AS 16.12.2020: changed 0101 to 1231
	task.CreateVirtualDatabase = False
	task.PerformTask 1, db.Count
	Set task = Nothing
	Set db = Nothing
oLog.LogMessage sEröffnungen & " created."
SetCheckpoint "analysis, Checkpoint 2.0"
	...
	task.AddFieldToInc "AUFLÖSUNG"
	task.AddFieldToInc "POSITION_SHORT"
	sAuflösungen = oSC.UniqueFileName("Auflösungen im aktuellen Bilanzjahr.IMD", FINAL_RESULT)
	task.AddExtraction sAuflösungen, "", "AUFLÖSUNG >= """ & sAktuelleGJAHR & "0101""  .AND. AUFLÖSUNG < """ & sAktuelleGJAHR & "1231""" 'AS 16.12.2020: changed 0101 to 1231
	task.CreateVirtualDatabase = False
	task.PerformTask 1, db.Count
	Set task = Nothing
	Set db = Nothing
oLog.LogMessage sAuflösungen & " created."
' 03.08.2022
SetCheckpoint "analysis, Checkpoint 3.0"
	...
	task.AddFieldToInc "AUFLÖSUNG"
	task.AddFieldToInc "POSITION_SHORT"
	sEröffnungenFolgeJahr = oSC.UniqueFileName("Eröffnungen nach dem aktuellen Bilanzjahr.IMD", FINAL_RESULT)
	task.AddExtraction sEröffnungenFolgeJahr, "", "ERÖFFNUNG > """ & sAktuelleGJAHR & "1231"""
	task.CreateVirtualDatabase = False
	task.PerformTask 1, db.Count
	Set task = Nothing
	Set db = Nothing
oLog.LogMessage sEröffnungenFolgeJahr & " created."
SetCheckpoint "analysis, Checkpoint 4.0"
	...
	task.AddFieldToInc "AUFLÖSUNG"
	task.AddFieldToInc "POSITION_SHORT"
	sAuflösungenFolgeJahr = oSC.UniqueFileName("Auflösungen nach dem aktuellen Bilanzjahr.IMD", FINAL_RESULT)
	task.AddExtraction sAuflösungenFolgeJahr, "", "AUFLÖSUNG > """ & sAktuelleGJAHR & "1231"" .AND. AUFLÖSUNG <> ""99991231"""
	task.CreateVirtualDatabase = False
	task.PerformTask 1, db.Count
	Set task = Nothing
	Set db = Nothing
oLog.LogMessage sAuflösungenFolgeJahr & " created."
end function
'-------------------------------------------------------------------------------------------------------------
