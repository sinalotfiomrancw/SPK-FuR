'-------------------------------------------------------------------------------------
' Title:		SK-FuR_0020_Sollbuchungen_auf_Habenkonten_in_EUR
' CIR:		SKA
' Customer:	Sparkassen
' Created by:	KKR
...
' ##############################################################################
		...
		Call Analysis()  ' aus frühenden  PS 09 (v1.3)
		Call Analysis2() ' aus frühenden  PS 15 (v1.3)
' ##############################################################################
' END INPUT
...
Dim dbSollAufHabenSummiert As String
...
Dim rec As Object
	...
	'dbBuchungenJeKtoRahmen = oSC.UniqueFileName("-SKA0020_Anzahl_Buchungen_je_bebuchten_KtoRahmen_mit_SHK.IMD")
	...
	dbBuchungenJeKtoRahmen = oSC.UniqueFileName(sAuditFolder & "-SKA0020_Anzahl_Buchungen_je_KtoRahmen.IMD")
	task.OutputDBName = dbBuchungenJeKtoRahmen
	task.CreatePercentField = False
	task.StatisticsToInclude = SM_SUM
	task.DisableProgressNotification = True
...
'	Set db = Client.OpenDatabase("dbUmsaetzeOBRManuell")
	...
	'dbName = oSC.UniqueFileName(sAuditFolder & "SK-FuR-0020_SollBuchungen_auf_HabenKonten.IMD")
	dbSollAufHaben = oSC.UniqueFileName(sAuditFolder & "SK-FuR-0020_SollBuchungen_auf_HabenKonten.IMD")
	...
	'task.AddExtraction dbName, "","AZ9_SALDO > 0,00 .AND. BETRAG <= 0,00 .AND. TEXTSCHLÜSSEL <> ""25"" .AND. TEXTSCHLÜSSEL <> ""68""" ' AS 06.10.2020
	'task.AddExtraction dbName, "","AZ9_SALDO >= 0,00 .AND. BETRAG < 0,00 .AND. TEXTSCHLÜSSEL <> ""25"" .AND. TEXTSCHLÜSSEL <> ""68""" ' AS 06.10.2020: Habenkonten mit Saldo 0,00
	'task.AddExtraction dbName, "","AZ9_SALDO >= 0,00 .AND. BETRAG < 0,00 .AND. TEXTSCHLÜSSEL <> ""25"" .AND. TEXTSCHLÜSSEL <> ""68""" & sPositionEqn ' 25.07.2022
	task.AddExtraction dbSollAufHaben, "","AZ9_SALDO >= 0,00 .AND. BETRAG < 0,00 .AND. TEXTSCHLÜSSEL <> ""25"" .AND. TEXTSCHLÜSSEL <> ""68""" & sPositionEqn ' 25.07.2022
...
'	Set oResultFile = oSC.CreateResultObject(dbName, FINAL_RESULT, True, 1)
	...
	Set oResultFile = oSC.CreateResultObject(dbSollAufHaben, INTERMEDIATE_RESULT, True, 2)
...
' Amount of rows which should be shown in the report
	...
	Call SetFlagForTable(dbSollAufHaben, TRUE)
	
	Set db = Client.OpenDatabase(dbSollAufHaben)
	Set task = db.Summarization
	task.AddFieldToSummarize "RAHMENNR_2STELLIG"
	task.AddFieldToTotal "BETRAG"
	'task.AddFieldToInc "WKZ"
	task.Criteria = "WKZ==""EUR"""
	dbSollAufHabenSummiert = oSC.UniqueFileName(sAuditFolder & "SK-FuR-0020_SollBuchungen_auf_HabenKonten_summiert_in_EUR.IMD")
	
	task.OutputDBName = dbSollAufHabenSummiert
	task.CreatePercentField = FALSE
	...
	'Set db = Client.OpenDatabase(dbSollAufHabenSummiert)
	'Set task = db.TableManagement
	'task.RemoveField "WKZ"
	'task.PerformTask
	'db.Close
	'Set task = Nothing
	'Set db = Nothing
	...
	If db.Count > 0 Then
		...
		task.CreateVirtualDatabase = False
		task.DisableProgressNotification = True
		dbSollAufHabenSummiert = oSC.UniqueFileName(sAuditFolder & "@SK-FuR-0020_SollBuchungen_auf_HabenKonten_in_EUR_je_KtoRahmen.IMD")
		task.PerformTask dbSollAufHabenSummiert, "", WI_JOIN_ALL_IN_PRIM
		db.Close
		Set task = Nothing
		Set db = Nothing
		...
		'Nicht funktinierende Verlinkungen von ActionsFelder entfernen	
		Call removeActionField(dbSollAufHabenSummiert, "ANZ_SAETZE_GESAMT")
	Else
		db.Close
		Set db = Nothing
	End If
	...
	'Final Result:
'	Set oResultFile = oSC.CreateResultObject(dbName, FINAL_RESULT, True, 1)
	Set oResultFile = oSC.CreateResultObject(dbSollAufHabenSummiert, FINAL_RESULT, True, 2)
' Short Description (optional)
'	oResultFile.ExtraValues.Add "ShortName", oGetResourceString("ResultFileSKA001ShortName")
' Description
'	oResultFile.ExtraValues.Add "Description", oGetResourceString("ResultFileSKA001Description")
...
' Amount of rows which should be shown in the report
	...
	Call SetFlagForTable(dbSollAufHabenSummiert, TRUE)
	
	If bPrivateMode = TRUE Then
		' Remove History in files created by Analysis
		If oSC.FileExists(dbSollAufHabenSummiert, "") Then oProtectIP.RemoveHistory dbSollAufHabenSummiert
	End If
...
'	Set db = Client.OpenDatabase("dbUmsaetzeOBRManuell")
	...
	'dbName = sAuditFolder & "SK-FuR-0020_SollBuchungen_auf_HabenKonten.IMD"
	...
	dbSollAufHabenSummiert = oSC.UniqueFileName(sAuditFolder & "@SK-FuR-0020_SollBuchungen_auf_HabenKonten_nicht_in_EUR_je_KtoRahmen.IMD")
	task.OutputDBName = dbSollAufHabenSummiert
	task.CreatePercentField = FALSE
	task.StatisticsToInclude = SM_SUM + SM_MAX + SM_MIN + SM_AVERAGE
	task.DisableProgressNotification = True
