'-------------------------------------------------------------------------------------
' Title:		SKA_010_Sollbuchungen_auf_Habenkonten_in_EUR
' CIR:		SKA
' Customer:	Sparkassen
' Created by:	KKR
...
Dim rec As Object
	...
	dbStornoHabenAufSollSummiert = "-SK-FuR-0021_Storno_HabenBuchungen_auf_SollKonten_in_EUR.IMD"
	...
	If db.Count > 0 Then
		...
		Call removeActionField("@SK-FuR-0021_Storno_HabenBuchungen_auf_SollKonten_in_EUR.IMD", "ANZ_SAETZE_GESAMT")
	...
	Call SetFlagForTable(dbStornoHabenAufSollSummiert, FALSE)
	...
	' ###############################################################################
	' # PS früher PS 16 jetzt Ergebniss als PS 21 ab v1.04 zusammengefügt 
	' # Local Variables
	' ###############################################################################
	...
	SmartContext.ScriptProgress 6, 40
	lNumberOfHistoryCountsSourceFileStart = oProtectIP.HistoryCount(dbNameSourceAdditional)
'	Set db = Client.OpenDatabase("dbUmsaetzeOBRManuell")
	Set db = Client.OpenDatabase(dbNameSourceAdditional)
	Set task = db.Summarization
	task.AddFieldToSummarize "RAHMENNR_2STELLIG1"
	task.AddFieldToTotal "BETRAG"
	dbStornoHabenAufSollSummiert = "@SK-FuR-0021_Storno_HabenBuchungen_auf_SollKonten_nicht_in_EUR.IMD"
	task.OutputDBName = dbStornoHabenAufSollSummiert
	task.CreatePercentField = FALSE
	task.StatisticsToInclude = SM_SUM + SM_MAX + SM_MIN + SM_AVERAGE
	task.DisableProgressNotification = True
